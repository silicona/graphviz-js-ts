(function($) {
        ...
        tooltips: {
            init: function($graph) {
                ...
                $a.tooltip({
                    ...
                    if ($a.attr('data-tooltip-keepvisible')) return false;
                });
            },
            show: function() {
                var $a = $(this);
    ...
    GraphvizSvg.prototype.init = function(type, element, options) {
        ...
        if (options.url) {
            ...
            if (options.svg) this.$element.html(options.svg);
            this.setup();
        }
    };
    ...
    GraphvizSvg.prototype.getOptions = function(options) {
        ...
        if (options.shrink) {
            if (typeof options.shrink != 'object') options.shrink = {
                x: options.shrink,
                y: options.shrink
            };
    ...
    GraphvizSvg.prototype.setup = function() {
        ...
        if (this.$background.length) this.$element.css('background', this.$background.attr('fill'));
        ...
        $title.remove();
        if (options.zoom) this.setupZoom();
        // tell people we're done
        if (options.ready) options.ready.call(this);
    };
    GraphvizSvg.prototype.setupNodesEdges = function($el, isNode) {
        ...
        $el.find('polygon, ellipse, path').each(function() {
            ...
            if (isNode && options.shrink) that.scaleNode($this);
        });
        ...
        if ($title[0]) {
            ...
            $title.remove();
            if (isNode) this._nodesByName[title] = $el[0];
            else this._edgesByName[title] = $el[0];
            // without a title we can't tell if its a user comment or not
            var previousSibling = $el[0].previousSibling;
            while(previousSibling && previousSibling.nodeType != 8)previousSibling = previousSibling.previousSibling;
            if (previousSibling != null && previousSibling.nodeType == 8) {
                ...
                if (value != title) // user added comment
        ...
        $el.children('a').filter(function() {
            ...
            if (options.tooltips) options.tooltips.init.call(this, that.$element);
        });
    };
    ...
    GraphvizSvg.prototype.scaleNode = function($node) {
        ...
        if (tagName == 'ellipse') {
            ...
            for(var i in pts){
                ...
                points += (cx - ox) / (bbox.width / 2) * dx + ox + ',' + ((cy - oy) / (bbox.height / 2) * dy + oy) + ' ';
            }
    ...
    GraphvizSvg.prototype.findEdge = function(nodeName, testEdge, $retval) {
        ...
        for(var name in this._edgesByName){
            ...
            if (match) {
                if ($retval) $retval.push(this._edgesByName[name]);
                retval.push(match);
            }
        }
        return retval;
    ...
    GraphvizSvg.prototype.colorElement = function($el, getColor) {
        ...
        $el.find('polygon, ellipse, path').each(function() {
            ...
            if (color.fill && $this.prop('tagName') != 'path') $this.attr('fill', getColor(color.fill, bg)) // don't set  fill if it's a path
            ;
            if (color.stroke) $this.attr('stroke', getColor(color.stroke, bg));
        });
    };
    GraphvizSvg.prototype.restoreElement = function($el) {
        $el.find('polygon, ellipse, path').each(function() {
            var $this = $(this);
            var color = $this.data('graphviz.svg.color');
            if (color.fill) $this.attr('fill', color.fill) // don't set  fill if it's a path
            ;
            if (color.stroke) $this.attr('stroke', color.stroke);
        });
    };
    ...
    GraphvizSvg.prototype.linkedTo = function(node, includeEdges) {
        ...
        this.findLinked(node, includeEdges, function(nodeName, edgeName) {
            var other = null;
            var match = '->' + nodeName;
            if (edgeName.endsWith(match)) other = edgeName.substring(0, edgeName.length - match.length);
            return other;
        }, $retval);
        return $retval;
    };
    GraphvizSvg.prototype.linkedFrom = function(node, includeEdges) {
        ...
        this.findLinked(node, includeEdges, function(nodeName, edgeName) {
            var other = null;
            var match = nodeName + '->';
            if (edgeName.startsWith(match)) other = edgeName.substring(match.length);
            return other;
        }, $retval);
        return $retval;
    };
    ...
    GraphvizSvg.prototype.tooltip = function($elements, show) {
        ...
        $elements.each(function() {
            $(this).children('a[title]').each(function() {
                if (show) options.tooltips.show.call(this);
                else options.tooltips.hide.call(this);
            });
        });
    };
    ...
    GraphvizSvg.prototype.sendToBack = function($elements) {
        if (this.$background.length) $element.insertAfter(this.$background);
        else $elements.detach().prependTo(this.$graph);
    };
    GraphvizSvg.prototype.highlight = function($nodesEdges, tooltips) {
        ...
        if ($nodesEdges && $nodesEdges.length > 0) {
            ...
            if (tooltips) this.tooltip($nodesEdges, true);
        } else {
...
//# sourceMappingURL=index.5a68d0d1.js.map
